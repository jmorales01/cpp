...
void lose()
{
PlaySound(TEXT("cool.wav"), NULL, SND_FILENAME | SND_ASYNC);
}
...
//gotoxy(x, y + 1); printf(" %c%c%c ", 40, 248, 41);
//gotoxy(x, y + 2); printf("  %c  ", 206);
gotoxy(x, y); printf("%c", 219);
gotoxy(x, y+1); printf("%c", 206);
gotoxy(x, y+2); printf("*");
...
void Avion::colisionNave(NAVE &nave)
{
...
n.pintarCorazones();
list<Avion*> A;
list<Avion*>::iterator itA;
int puntos = 0;

for (int i = 0; i < 5; i++)
{
A.push_back(new Avion(rand() % 75 + 3, rand() % 5 + 4));
}
...
delete (*it);
it = B.erase(it);
A.push_back(new Avion(rand() % 75 + 3, 4));
gotoxy((*itA)->X(), (*itA)->Y());
printf(" ");
delete (*itA);
itA = A.erase(itA);
